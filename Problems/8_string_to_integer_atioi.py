...
We strip all left white spaces. We then check if there's a leading '-' or '+' which the former setting neg = True, in both cases we then consider all characters after the optional operator.
Then we iterate through the characters and update the value of num or break the loop if it's not a digit. If num  doesn't change, then we return since the first character wasn't a digit.

(note if there are leading zeros, we should check that)

Then we return the sign to the number. We then check if the number is within the range 2^-31 < num < 2^31 - 1 and return num if it is or we return bounds if it's not
"""